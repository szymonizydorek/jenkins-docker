...
func TestFullRune(t *testing.T) {
	for _, m := range utf8map {
		...
		s := m.str
		if !FullRune(s) {
			t.Errorf("FullRune(%q) (%U) = false, want true", s, m.r)
		}
		b1 := b[0 : len(b)-1]
		if FullRune(b1) {
...
func TestDecodeSurrogateRune(t *testing.T) {
	for _, m := range surrogateMap {
		...
		r, size = DecodeRune(s)
		if r != RuneError || size != 1 {
			t.Errorf("DecodeRune(%q) = %x, %d want %x, %d", s, r, size, RuneError, 1)
		}
	}
}
...
func TestRuntimeConversion(t *testing.T) {
	for _, ts := range testStrings {
		...
		if n := runtimeRuneCount(ts); n != count {
			t.Errorf("%q: len([]rune()) counted %d runes; got %d from RuneCount", ts, n, count)
			break
		}

		runes := []rune(ts)
		if n := len(runes); n != count {
			t.Errorf("%q: []rune() has length %d; got %d from RuneCount", ts, n, count)
			break
		}
		i := 0
		for _, r := range ts {
...
func TestDecodeInvalidSequence(t *testing.T) {
	for _, s := range invalidSequenceTests {
		...
		if r1 != r2 {
			t.Errorf("DecodeRune(%#x) = %#04x mismatch with DecodeRune(%q) = %#04x", s, r1, s, r2)
			return
		}
...
func testSequence(t *testing.T, s string) {
	...
	for si = len(s); si > 0; {
		...
		r2, size2 := DecodeLastRune(s[0:si])
		if size1 != size2 {
...
func TestRuneCount(t *testing.T) {
	for _, tt := range runecounttests {
		if out := RuneCount(tt.in); out != tt.out {
...
func BenchmarkRuneCountStringTenASCIIChars(b *testing.B) {
	for i := 0; i < b.N; i++ {
		RuneCount("0123456789")
	}
}

func BenchmarkRuneCountStringTenJapaneseChars(b *testing.B) {
	for i := 0; i < b.N; i++ {
		RuneCount("日本語日本語日本語日")
	}
}
